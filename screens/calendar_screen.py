...
class AgendaTaskCard(RoundedThemePanel):
    ...
    def __init__(self, task, on_completed=None, on_open=None, **kwargs):
        ...
        # Opens the Smart Planner's ActivityDetailScreen for this task --
        # this is the piece that makes the calendar part of the linked
        # planner instead of a standalone task list. Kept as a separate
        # button (rather than making the whole card tappable) so it
        # doesn't fight with the complete-checkbox or link-button taps.
        if self.on_open:
            details_btn = ThemeButton(
                text="Details \u203a",
                size_hint_x=None,
                width=dp(84),
                height=dp(36),
            ...
            details_btn.bind(on_release=self._open_detail)
...
class CalendarScreen(ThemedScreenMixin, Screen):
    ...
    def build_calendar(self):
        ...
        task_dates = set(
            get_all_task_dates(self.current_year, self.current_month, self._user_id())
        )
    ...
    def select_date(self, date_value):
        ...
        self.current_tasks = get_tasks_by_date(date_value, self._user_id())
    ...
    def open_task_detail(self, task_id):
        """
        Routes a tapped calendar task to whichever screen makes sense for
        its activity_type -- the same pattern HomeScreen uses for its
        today's-plan rows. There's no separate ActivityDetailScreen
        anymore (it was removed), so this replaces that dead end.
        """
        if not task_id or not self.manager:
            return
        ...
        if activity_type == "study":
            ...
            editor.current_note_id = notes[0][0] if notes else None
            self.manager.current = "note_editor"

        # For "event" / "task" activity types we're already on the
        # calendar screen looking at this task, so there's nothing
        # further to navigate to.
    ...
    def on_pre_enter(self, *_args):
        # Rolls forward any incomplete carry_forward task whose due_date
        # has already passed, before the calendar renders -- this is
        # what actually implements the "push incomplete task to next
        # date" behavior the carry_forward toggle promises.
        carry_forward_incomplete_tasks(self._user_id())
        self.refresh_category_filters()
        self.build_calendar()
        self.select_date(self.selected_date)
    ...
    def open_add_task_popup(self, *_args, activity_type=None):
        """Open the redesigned themed task editor."""
        self._popup_activity_type = activity_type or "task"
        ...
        save_btn.bind(
            on_release=lambda _button: self.save_task_from_popup(
                ...
                activity_type=self._popup_activity_type,
            )
        )
        popup.open()
